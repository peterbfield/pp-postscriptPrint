...
tree_t tree_srch(ppr_tree, pfi_compare, p_user)
   tree			**ppr_tree;
   int			(*pfi_compare)();
   tree_t		p_user;
{
   ...
   if (*ppr_tree)
   {
      ...
      if (i_comp > 0)
	 return(tree_srch(&(**ppr_tree).tree_r,pfi_compare,p_user));

      if (i_comp < 0)
	 return(tree_srch(&(**ppr_tree).tree_l,pfi_compare,p_user));
...
void tree_add(ppr_tree, pfi_compare, p_user, pfv_uar)
   tree			**ppr_tree;
   int			(*pfi_compare)();
   tree_t		p_user;
   void			(*pfv_uar)();
...
int tree_delete(ppr_p, pfi_compare, p_user, pfv_uar)
   tree			**ppr_p;
   int			(*pfi_compare)();
   tree_t		p_user;
   void			(*pfv_uar)();
...
void tree_mung(ppr_tree, pfv_uar)
   tree			**ppr_tree;
   void			(*pfv_uar)();
{
   if (*ppr_tree)
...
static void sprout(ppr, p_data, pi_balance, pfi_compare, pfv_delete)
   tree	**ppr;
   tree_t		p_data;
   int			*pi_balance;
   int			(*pfi_compare)();
   void			(*pfv_delete)();
{
   ...
   if (cmp < 0) 
   {
      ...
      sprout(&(*ppr)->tree_l, p_data, pi_balance,pfi_compare, pfv_delete);
      if (*pi_balance) 
	 ...
	 switch ((*ppr)->tree_b)
	 {
	    ...
	    case 0:	/* balance WAS okay; now left branch longer */
	       ...
	       (*ppr)->tree_b = -1;
	       break;
	    case -1: /* left branch was already too long. rebalnce */
   ...
   if (cmp > 0) 
   {
      ...
      sprout(&(*ppr)->tree_r, p_data, pi_balance,pfi_compare, pfv_delete);
      if (*pi_balance) 
      {
...
static int delete(ppr_p, pfi_compare, p_user, pfv_uar, pi_balance, pi_uar_called)
   tree			**ppr_p;
   int			(*pfi_compare)();
   tree_t		p_user;
   void			(*pfv_uar)();
   ...
   int			*pi_uar_called;
{
   ...
   if (i_comp > 0) 
   {
      ...
      i_ret = delete(&(*ppr_p)->tree_l, pfi_compare, p_user, pfv_uar,
	 ...
	 i_ret = delete(&(*ppr_p)->tree_r, pfi_compare, p_user, pfv_uar,
			pi_balance, pi_uar_called);
	 if (*pi_balance)
	 ...
	 if (pr_q->tree_r == NULL) 
	 {
	    ...
	    *pi_balance = TRUE;
	    ...
	    else 
	    {
	       ...
	       del(&pr_q->tree_l, pi_balance, &pr_q, pfv_uar,
		   pi_uar_called);
	       if (*pi_balance)
		  bal_L(ppr_p, pi_balance);
	    }
...
static void del(ppr_r, pi_balance, ppr_q, pfv_uar, pi_uar_called)
   tree			**ppr_r;
   int			*pi_balance;
   tree			**ppr_q;
   void			(*pfv_uar)();
   int			*pi_uar_called;
{
   ...
   {
      del(&(*ppr_r)->tree_r, pi_balance, ppr_q,
...
static void bal_L(ppr_p, pi_balance)
   tree			**ppr_p;
   int			*pi_balance;
{
   tree			*p1, *p2;
...
static void bal_R(ppr_p, pi_balance)
   tree			**ppr_p;
   int			*pi_balance;
{
   tree			*p1, *p2;
